...
bool BinaryOperationMatcher::checkIsBinaryOperator(parserProgress &p){
    return p.peekSymbol(binary_operators)||p.peekKeyword(binary_keyword_operators);
}
...
std::shared_ptr<BinaryOperation> BinaryOperationMatcher::makeMatch(parserProgress &p){
    ...
    if(!(kptr=p.isKeyword(binary_keyword_operators))){
        if(!(sptr=p.isSymbol(binary_operators))){
    ...
    if(sptr){
        return std::make_shared<BinaryOperation>(term1,sptr,term2,line_start,p.get_line(-1));
    }else{
        return std::make_shared<BinaryOperation>(term1,kptr,term2,line_start,p.get_line(-1));
    }
}

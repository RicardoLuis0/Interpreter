...
bool CharType::is(std::shared_ptr<Type> self,std::shared_ptr<Type> other){
    return CHECKPTR(other,CharType)||Type::is(self,other);
}

bool CharType::allows_implicit_cast(std::shared_ptr<Type> self,std::shared_ptr<Type> other){
    return CHECKPTR(other,UnsignedCharType)||CHECKPTR(other,UnsignedIntType)||CHECKPTR(other,IntType)||CHECKPTR(other,FloatType)||Type::allows_implicit_cast(self,other);
}

std::shared_ptr<Value> CharType::cast(std::shared_ptr<Value> self,std::shared_ptr<Type> other){
...
std::shared_ptr<Value> CharType::get_operator_result(int op,std::shared_ptr<Value> self,std::shared_ptr<Value> other,int line_start,int line_end){
    ...
    case SYMBOL_LOGICAL_OR:
        if(other->get_type()->allows_implicit_cast(other->get_type(),Type::char_type())){
            return std::make_shared<DummyValue>(Type::char_type());
             ...
             other->get_type()->allows_implicit_cast(other->get_type(),Type::int_type())||
    ...
    case SYMBOL_DIVIDE:
        ...
        }else if(other->get_type()->is(other->get_type(),Type::char_type())){
            return std::make_shared<DummyValue>(Type::char_type());
        }else if(other->get_type()->allows_implicit_cast(other->get_type(),Type::unsigned_int_type())||
             other->get_type()->allows_implicit_cast(other->get_type(),Type::int_type())||
             other->get_type()->allows_implicit_cast(other->get_type(),Type::unsigned_char_type())){
    ...
    case SYMBOL_DIVIDE_ASSIGNMENT:
        if(other->get_type()->allows_implicit_cast(other->get_type(),Type::int_type())||
            ...
            return std::make_shared<DummyVariable>(Type::char_type());
        }else{
            throw MyExcept::SyntaxError(line_start,line_end,"incompatible types "+self->get_type()->get_name()+" and "+other->get_type()->get_name()+" for operator '"+get_op_str(op)+"'");
        }
...
std::shared_ptr<Value> CharType::get_unary_operator_result(int op,std::shared_ptr<Value> self,bool pre,int line_start,int line_end){
...
std::shared_ptr<Value> CharType::call_operator(int op,std::shared_ptr<Value> self,std::shared_ptr<Value> other){
    switch(op){
    case SYMBOL_PLUS:
    case SYMBOL_MINUS:
    case SYMBOL_MULTIPLY:
...
std::shared_ptr<Value> CharType::call_unary_operator(int op,std::shared_ptr<Value> self,bool pre){
    if(pre){
        switch(op){
        default:

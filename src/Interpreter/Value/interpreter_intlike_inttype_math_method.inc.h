...
std::shared_ptr<Value> CLASS_NAME::METHOD_NAME(std::shared_ptr<Value> &other){
    #ifdef UNSIGNED
    if CHECK2(UnsignedIntValue,v3) {
        return std::make_shared<UnsignedIntValue>(static_cast<unsigned int>(VALUE_NAME OPERATOR v3->get()));
    }else if CHECK2(UnsignedCharValue,v4) {
        #if CLASS_NAME == UnsignedCharValue
        return std::make_shared<UnsignedCharValue>(static_cast<unsigned char>(VALUE_NAME OPERATOR v4->get()));
        #else
    ...
    if CHECK2(IntValue,v1) {
        ...
        #else
        return std::make_shared<IntValue>(static_cast<int>(VALUE_NAME OPERATOR val->get()));
        #endif
    }else{

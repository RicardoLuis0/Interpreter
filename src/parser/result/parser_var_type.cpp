...
VarType::VarType(std::shared_ptr<Lexer::KeywordToken> tk,bool c,bool h,bool s,int ls,int le):VarType(
...
VarType::VarType(ParserPrimitiveType_t p,bool c,bool h,bool s,int ls,int le):ParserResultPart(ls,le),primitive(p),type(VARTYPE_PRIMITIVE),has_sign(h),sign(s),is_const(c){
}

...
class linker_ld(linker):
    ...
    def __init__(self,flags,libs,start_files,end_files):
        super(linker_ld,self).__init__(start_files,end_files)
...
class linker_gcc(linker):

    def __init__(self,cpp,flags,libs,start_files,end_files):
        super(linker_gcc,self).__init__(start_files,end_files)
        self.flags=flags #linker flags, list
        self.libs=libs #linker libraries, list
        self.cpp=cpp
    ...
    def get_cmd(self,out): #get command to run
        ...
        cmd=compiler if not gcc_path else "\""+gcc_path+compiler+"\""
        args=(" ".join(self.flags))+" "+(" ".join(self.libs))+" -o \""+out+"\" "+(" ".join(self.start_files))+" "+(" ".join(self.files))+" "+(" ".join(self.end_files))
        return cmd+" "+args
...
class gcc_base(compiler): #gcc compiler
    def __init__(self,defines,flags,incremental,cpp):
        self.defines=defines
        self.flags=defines+flags #gcc flags, list
        self.cpp=cpp #whether to run gcc or gcc++
...
class gcc_cpp(gcc_base):
    def __init__(self,defines,flags,incremental=True):
        gcc_base.__init__(self,defines,flags,incremental,True)

class gcc_c(gcc_base):
    def __init__(self,defines,flags,incremental=True):
        gcc_base.__init__(self,defines,flags,incremental,False)
